...
const EMPTY_FORM = {
  ...
  Logs: '', PaymentDueDate: '', PaymentStatus: 'Pending', PartnerShares: '',
};
...
export default function Projects() {
  ...
  // Parse PartnerShares JSON string → object, e.g. '{"ดาม":40,"กฤต":60}' → {ดาม:40,กฤต:60}
  const parseShares = (sharesStr) => {
    try { return sharesStr ? JSON.parse(sharesStr) : {}; } catch { return {}; }
  };

  const setShare = (partnerName, value) => {
    const current = parseShares(form.PartnerShares);
    current[partnerName] = value === '' ? '' : parseFloat(value) || 0;
    setForm({ ...form, PartnerShares: JSON.stringify(current) });
  };

  const totalShares = () => {
    const obj = parseShares(form.PartnerShares);
    ...
    <div className="p-6">
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50 border-b border-gray-100">
                      <tr>
                        ...
                        <th className="text-center px-4 py-3 text-xs font-semibold text-gray-500 uppercase">สัดส่วน</th>
                        ...
                        <tr key={p.ProjectID} className="hover:bg-gray-50 group">
                          ...
                          <td className="px-4 py-3 text-center">
                            {p.PartnerShares
                              ? (() => {
                                  try {
                                    const obj = JSON.parse(p.PartnerShares);
                                    return (
      ...
      >
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="col-span-2">
            <label className="block text-sm font-medium text-gray-700 mb-2">สัดส่วนกำไร (%)</label>
            {partners.length === 0 ? (
              <p className="text-xs text-gray-400">ยังไม่มีหุ้นส่วน กรุณาตั้งค่าหุ้นส่วนก่อน</p>
            ) : (
              <div className="space-y-2">
                {partners.map(p => {
                  const shares = parseShares(form.PartnerShares);
                  return (
                    <div key={p.PartnerID} className="flex items-center gap-3">
                      <span className="text-sm text-gray-700 w-24 flex-shrink-0">{p.Name}</span>
                      <div className="relative flex-1">
                        <input
                          ...
                          placeholder="0"
                          value={shares[p.Name] ?? ''}
                          onChange={e => setShare(p.Name, e.target.value)}
                        />
                ...
                {(() => {
                  ...
                  return (
                    <p className={`text-xs mt-1 ${Math.abs(total - 100) < 0.01 ? 'text-green-600' : total > 100 ? 'text-red-500' : 'text-amber-600'}`}>
                      รวม: {total}% {Math.abs(total - 100) < 0.01 ? '✅' : total > 100 ? '⚠️ เกิน 100%' : `⚠️ ขาดอีก ${100 - total}%`}
                    </p>
                  );
                })()}
              </div>
